...
class Request:
    """
    Object used to make requests to external APIs
    """
    def __init__(self):
        self._httpx = httpx
        self.urls = {
            "cocktail_db_api": "https://www.thecocktaildb.com/api/json/v1/1/random.php",
            "battle_music": "https://www.youtube.com/watch?v=lAGm9MTyRJ8",
        }

    def get_drink_json(self):
        return self._httpx.get(self.urls["cocktail_db_api"]).json()["drinks"][0]
...
class DrinkEmbedder:
    ...
    def __init__(self, drink):
        self.drink = drink
        embed = Embed(
            title=self.drink._name,
            description="A drink for you, friend...",
    ...
    def embed_drink(self, embed):
        ...
        embed.add_field(
            name="Instructions", value=self.drink._instructions, inline=False
        )
        embed.set_footer(
...
class PinEmbedder:
    ...
    def __init__(self, pin):
        self.pin = pin
        embed = Embed(
            title=self.pin.author,
            description=f"Posted on {self.pin.posted_date}",
            color=int(Colors().user_colors.get_user_color(self.pin.id)),
        )
    ...
    def embed_pin(self, embed):
        ...
        if self.pin.content is not None:
            pin_embed.add_field(
                name="Message",
                value=f"[{self.pin.content}]({self.pin.url})",
                inline=False,
            )
        else:
            pin_embed.add_field(
                name="Message", value="*This pin had no message*", inline=False
            )
        return pin_embed
...
class UserColors(Configurable):
    """
    Configuration Child Object which stores the color values for each user
    in the server
    """
    def __init__(self):
...
class Colors(Configurable):
    """
    Configuration Child which stores the values of colors which can be accessed globally.
    """
    def __init__(self):
        Configurable.__init__(self)
